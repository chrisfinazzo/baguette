...
### Added
- **`app-switcher` and `swipe-to-home` virtual buttons.** Two new `Press`-compatible wire names route through `baguette button --udid …`, the `/event` HTTP route, the browser's `simInput.button(...)` bridge, and the focus-mode toolbar. `app-switcher` decomposes into two consecutive home `IndigoHIDMessageForButton` presses ~150 ms apart — SpringBoard listens to that event source regardless of whether the device has a physical home button, so it works on Face ID iPhones (recipe matches idb's `FBSimulatorPurpleHID`). `swipe-to-home` rides a *new* 7-arg `IndigoHIDMessageForMouseNSEvent` dispatch path that flags every touch with `IndigoHIDEdge.bottom (= 3)`, which is what tells the iOS HID stack to route the events to the home-indicator gesture recognizer instead of the foreground app's pan handlers. Disassembling SimulatorKit on Xcode 26 confirmed the real ABI is `(CGPoint*, CGPoint*, target, eventType, edge, NSSize.w, NSSize.h)` — `cmp x24, #0x4` in the function prologue is bounds-checking `IndigoHIDEdge`'s 5 cases, and `SimDigitizerInputView.TouchEvent.edge: __C.IndigoHIDEdge` exposes the same flag in SimulatorKit's public Swift surface. The legacy 9-arg dispatcher used for taps / pans / pinches stays untouched (verified in production); only edge gestures route through the corrected signature.

### Fixed
- **App-switcher button no longer locks the screen ([#5](https://github.com/tddworks/baguette/issues/5)).** The third icon in the focus-mode toolbar carries the two-overlapping-squares glyph that mirrors `Simulator.app`'s app-switcher button, but it was wired to `simInput.button('lock')` — every click locked the device instead. The click now fires `simInput.button('app-switcher')` against the new virtual button described above. Lock remains reachable via `baguette button lock --udid …` and the legacy `/event` HTTP route; the focus-mode toolbar now matches `Simulator.app`'s Home / Screenshot / App Switcher trio.
