...
- **A HID target is only ever a constant some create-service message registered — never a computed number.** `SimHIDVirtualServiceManager` keeps registered services in a dictionary and **throws** on an unknown target, killing `backboardd` and SpringBoard with it (it presents as the simulator spontaneously rebooting). The guest names the valid set when it dies: `'Encountered HID event with unexpected target 1073741826 not in known targets: (50, 13, 11, 53, 51, 302, 300, 1, 14, 60, 12, 100, 54, 1073741825, 301)'` — `50` = `0x32` phone, `53` = `0x35` pointer, `54` = `0x36` mouse, `1073741825` = `0x40000001` CarPlay. **`IndigoHIDTargetForScreen` is a trap**: it is a real SimulatorKit export returning `0x40000000 | screenId`, which looks like the answer and is a target nothing registered (`0x40000002` above is screen 2). CarPlay is a *fixed* `0x40000001` because `IndigoHIDMessageToCreateCarPlayService` hardcodes `1` into the target slot at `[0x40]`, exactly as its siblings hardcode theirs. That service must also be **created** before use — `warmServices` did pointer and mouse but not CarPlay; its one byte is `hasTouchScreen`, a BOOL, not a screen index (from Simulator.app's `[[self starkConfig] hasTouchScreen]`; "Stark" is Apple's codename for CarPlay). Opcode `[0x30]`: 3 pointer / 5 mouse / 7 CarPlay / 8 remove-CarPlay. See [`docs/features/companion-screens.md`](docs/features/companion-screens.md).
